...
void Tetris::setOperation(char key, TetrisState s0, ActionHandler *h1,
  ...
  else {           //등록된 operation이 아닌 경우  + 꽉찬 경우 
    if (nOps == MAX_TETRIS_OPERATIONS) {        // 꽉찬 경우, nops를 통해 비교 
...
void Tetris::init(int **setOfBlockArrays, int nTypes, int nDegrees) {
  ...
  setOfBlockObjects = new Matrix**[numTypes]; // allocate 1d array of pointers to Matrix pointer array
...
void Tetris::update_oScreen(Matrix *tempBlk, int y, int x) {
  ...
  oScreen->paste(tempBlk, y, x); // y: top, x:left (pasting tempblk;)
}

TetrisState Tetris::accept(char key) {
  int idx = findOpIdxByKey(key);
  if (idx == -1) {  //존재하지 않는 operation
