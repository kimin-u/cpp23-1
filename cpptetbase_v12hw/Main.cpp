...
char getTetrisKey(TetrisState state, bool fromUser, bool toFile) {
  ...
  //replay
    ...
    if (infStream.eof() == true)
      ...
      if (keygroup.empty()){
        char tmp[10];
        infStream.read(tmp,10);
        keygroup.assign(tmp,infStream.gcount());
      }
      key = keygroup.front();
      keygroup.erase(0,1);
  ...
  //record
    ...
    keygroup.push_back(key);
    if (keygroup.size()==10){
      outfStream.write(keygroup.c_str(), 10);
      keygroup.clear();
    }
    //outfStream.write(keygroup.c_str(),10);
    //outfStream<<key;
...
int main(int argc, char *argv[]) {
  ...
  if (outfStream.is_open() == true) {
    outfStream.write(keygroup.c_str(), keygroup.size());
    outfStream.close();
  }
